...
def run_copy(job_id: int, local_db: Session) -> None:
    ...
    Copies files from contentstore to export directory.
    Commits after every file so the frontend sees live progress.
    """
    ...
    logger.info("Copying %d files for job %d", len(pending_records), job_id)

    for idx, record in enumerate(pending_records):
        try:
            src_path = _resolve_content_path(record.content_url)
            if not src_path.exists():
                raise FileNotFoundError(f"Content file not found: {src_path}")

            dest_path = _build_dest_path(job.site_name, record.full_path, record.file_name)
            _safe_mkdir(dest_path.parent)

            t0 = time.perf_counter()
            shutil.copy2(src_path, dest_path)  # overwrites if already exists
            elapsed = time.perf_counter() - t0

            record.status = FileStatus.copied
            record.local_export_path = str(dest_path)
            record.error_msg = None
            file_bytes = record.file_size_bytes or 0
            record.transfer_speed_bps = int(file_bytes / elapsed) if elapsed > 0 else None
            job.copied_files += 1
            job.copied_size_bytes += file_bytes

        except Exception as exc:
            logger.exception("Failed copying node %s: %s", record.node_ref, exc)
            record.status = FileStatus.failed
            record.error_msg = str(exc)
            job.failed_files += 1

        # Commit after every file so the frontend polls see live progress
        job.updated_at = datetime.utcnow()
        local_db.commit()
        if (idx + 1) % 10 == 0:
            logger.info(
                "Copied %d/%d files, %s bytes so far…",
                job.copied_files, len(pending_records), job.copied_size_bytes,
            )

    # Regenerate final CSV from DB so local_export_path is fully populated for all records
    try:
...
def _safe_mkdir(path: Path) -> None:
    ...
    try:
        ...
        # If the directory already exists, ignore
        if path.is_dir():
            return
        raise
...
def _build_dest_path(site_name: str, full_path: str, file_name: str) -> Path:
    ...
    """
    # full_path is like /Folder/Sub/filename.ext — use all but the last segment as folder
