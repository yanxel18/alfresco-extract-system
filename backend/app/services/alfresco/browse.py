...
from .shortcuts import _detect_shortcut_types, resolve_shortcut_targets
...
def get_folder_children(db: Session, parent_node_id: int) -> dict:
    ...
    Resolves app:filelink shortcuts that target real files.
    Folder-target shortcuts are shown as shortcut entries, but are not expanded inline.
    """
    ...
    if filelink_ids:
        ...
        target_by_shortcut = resolve_shortcut_targets(db, filelink_ids)
        target_ids = sorted(set(target_by_shortcut.values()))
        if target_ids:
            finfo_placeholders = ", ".join(f":tfid_{i}" for i in range(len(target_ids)))
            finfo_sql = text(f"""
                SELECT
                    np.node_id,
                    mt.mimetype_str,
                ...
                WHERE np.node_id IN ({finfo_placeholders})
                  AND np.qname_id = :content_qid
            """)
            fparams = {f"tfid_{i}": nid for i, nid in enumerate(target_ids)}
            fparams["content_qid"] = content_qid
            target_file_info = {
                r.node_id: {
                    "mime_type": r.mimetype_str,
            ...
            for shortcut_id, target_id in target_by_shortcut.items():
                info = target_file_info.get(target_id)
                if info is not None:
                    filelink_info[shortcut_id] = info

    # True folder node_ids (exclude filelink shortcuts that resolved to files)
    true_folder_ids = [nid for nid in folder_node_ids if nid not in filelink_info]
    ...
    folders = [
        {
            ...
            "has_children": nid in folders_with_children,
            "is_shortcut": nid in shortcut_type_map,
            "selectable": nid not in shortcut_type_map,
        }
